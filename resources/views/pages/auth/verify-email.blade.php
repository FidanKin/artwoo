...
<x-layout.standard>
    <section class="my-28">
        @if(session('message'))
            <x-shared.notification.simple.success id="email-notification" message="{{ session('message') }}" />
        @endif

        <div class="mb-4 text-lg">
            {{ __('auth.confirm_account_description') }}
        </div>

        <div class="mt-4 flex items-center justify-between">
            <form method="POST" action="{{ route('verification.send') }}">
                @csrf
                <x-shared.core.button text="{{ __('auth.resend_confirm_email') }}" />
            </form>
        </div>
    </section>
