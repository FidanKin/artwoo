...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Страница автора - {{ $author['login'] }}</title>
        <!-- Fonts -->
    ...
    <body class="font-roboto">
        <x-layout.standard bgGray="{{ true }}" :navigationNodes="$navigation">
            <x-widgets.authorMain canSendMessage="{{ $can_send_message }}" :$author :$artworks :userNavigation="$user_navigation" :isUserSelf="! $can_contact" />
        </x-layout.standard>
        @vite('resources/js/app.js')
        @vite('resources/js/pages/my.js')
