...
<head>
    ...
    <title>Модерация авторских работ</title>
    <!-- Fonts -->
...
<x-layout.standard bgGray={{ true }}>
    ...
    <x-layout.secondary.content-base>
        <x-slot:headerSlot>
            <h2 class="text-h2">Модерация авторских работ</h2>
        </x-slot:headerSlot>
        ...
        <div class="flex flex-col flex-wrap">
            @foreach($artworks as $artwork)
                <div class="mb-5 border-b-2 p-2 border-black">
                    <form method="POST" action="/admin/moderation/artworks">
                        <input type="hidden" name="target_user" value="{{ $artwork['artwork']->user_id }}">
                        <input type="hidden" name="object_id" value="{{ $artwork['artwork']->id }}">
                        @csrf
                        <div class="flex flex-col">
                            <p class="font-bold">Artwork info:</p>
                            <div class="min-info border rounded p-4">
                                <p>ID -> {{ $artwork['artwork']->id }}</p>
                                <p>Name -> {{ $artwork['artwork']->name }}</p>
                                <p>Description -> {{ $artwork['artwork']->description }}</p>
                                <div class="flex flex-row flex-wrap gap-2">
                                    @foreach($artwork['images'] as $image)
                                        <img src="{{ $image }}" class="w-56 h-auto object-contain" />
                                    @endforeach
                                </div>
                            </div>
